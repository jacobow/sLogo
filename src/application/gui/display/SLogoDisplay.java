...
public class SLogoDisplay extends SLogoUIAreaWithFileIO implements Display {
	...
	private boolean effectsTracker;
	...
	private void initialize(){
		...
		displayToolbar.getItems().add(makeButton("Effects On",e->toggleTurtleEffects()));
	...
	private void toggleTurtleEffects() {
		if(effectsTracker){
			for(Turtle t : myTurtleList){
				myTurtleMap.get(t).getListener().turnOffSelectionEffects();
			}
			effectsTracker=false;
		}
		else{
			for(Turtle t : myTurtleList){
				myTurtleMap.get(t).getListener().turnOnSelectionEffects();
			}
			effectsTracker=true;
		}
...
//		Activate turtles
		...
		for(int i=0;i<myTurtleList.size();i++){
			if(myTurtleList.get(i)!=null && myTurtleList.get(i).getState().getActiveProperty().getValue()){
				fileWriter.print(i+" ");
			}
		}
		fileWriter.println("]");
