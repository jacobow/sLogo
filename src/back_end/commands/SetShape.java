...
public class SetShape extends ExecutableCommand {

	public SetShape(int arraySize, String userInput) {
	...
	public Constant execute(Model model) {
		for(Turtle t : model.getTurtles()) {
			if(t.getState().isActive()) t.getState().setTurtleShape(getValueArray()[0]);
		}
